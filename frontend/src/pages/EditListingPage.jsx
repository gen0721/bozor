...
export default function EditListingPage({ categories }) {
  const { id } = useParams();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    listingsAPI.getById(id)
      .then(res => {
        const l = res.data;
        if (l.user_id !== user.id) { navigate('/'); return; }
        setForm({
          title: l.title,
  ...
  const removeImageField = (idx) => {
    if (images.length > 1) setImages(prev => prev.filter((_, i) => i !== idx));
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span>/</span>
        <Link to={`/listings/${id}`} className="hover:text-primary-600 transition-colors">{t('listings')}</Link>
        <span>/</span>
        <span className="text-gray-600">{t('editListing')}</span>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-primary-600 to-primary-700 px-6 py-5">
          <h1 className="text-xl font-bold text-white">{t('editListingTitle')}</h1>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {/* Status toggle */}
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            <span className="text-sm font-medium text-gray-700">Статус объявления</span>
            <button
              type="button"
          ...
          {/* Images */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">{t('listingImages')}</label>
            ...
            {images.length < 6 && (
              <button type="button" onClick={addImageField} className="mt-2 flex items-center gap-1.5 text-primary-600 hover:text-primary-700 text-sm font-medium transition-colors">
          ...
          <div className="flex gap-3 pt-2">
            <Link to={`/listings/${id}`} className="btn-secondary flex-1 text-center py-3">
              {t('cancel')}
            </Link>
            <button type="submit" disabled={loading} className="btn-primary flex-1 py-3 text-base">
              {loading ? t('loading') : t('saveListing')}
            </button>
          </div>
        </form>
      </div>
